...
INPUT_CSV_PATH = "/Users/johnesposito/Downloads/HumanTranscriptions100Pages - Sheet1 (3).csv"  # Fill in path to input CSV
OUTPUT_CSV_PATH = "/Users/johnesposito/Documents/GitHub/Project-BlueBook-AI-OCR-Correction/OCR Evaluation/Results/gpt4o_eval_output.csv"
...
def get_gpt4o_correction(ocr_text, user_prompt):
    # Ensure OCR text is always a string
...
for idx, row in df.iterrows():
    ...
    for prompt_idx, prompt_text in enumerate(PROMPTS, start=1):
        for trial in range(1, 4):
            ...
            correction = get_gpt4o_correction(ocr_text, prompt_text)
            elapsed = round(time.time() - start_time, 2)

            # CER only if Human Text is non-empty
            if human_text.strip():
                cer = string_comparison.character_error_rate(correction, human_text)
            else:
                cer = None  # Mark as N/A if missing
